...
#         input_dim = self.input_dim  # calculated in Base from embedding and non-embedded dims
#         act = nn.ReLU()
...
#     def forward_with_embeddings(self, x, embs, return_shape='task', apply_transform=True):
#         task_idx = x[:, [2]]
#         x = self.embed_input(x, embs)
#         x = self.nonlinear(x)
#         x = self.readout(x)

#         if apply_transform and self.output_transform:
#             x = self.output_transform(x)

#         if return_shape == 'all':
#             return x
#         elif return_shape == 'task':
#             return x.gather(1, task_idx)
#         else:
#             raise ValueError(f"Unknown return_shape: {return_shape}")
...
#         vocab_size=None,
#         non_embedded_input_dim=None,
#         **kwargs
#     ):
#         super(SimpleMLP, self).__init__()

#         if input_dim is None:
#             input_dim = len(vocab_size) + (non_embedded_input_dim or 0)
...
#         x = x.float()  # <--- Cast input to float here
#         x = self.nonlinear(x)
#         x = self.readout(x)
...
# class SimpleSequentialMLP(nn.Module):
#     def __init__(
#         self,
#         hidden_dim: int,
#         input_dim: int = None,
#         output_dim: int = 1,
#         depth: int = 5,  # Adjust depth to maintain comparable number of parameters
#         output_transform=None,
#         vocab_size=None,
#         non_embedded_input_dim=None,
#         **kwargs
#     ):
#         super(SimpleSequentialMLP, self).__init__()
...
def get_model_fn(config):
    if config.MODEL == "baseline":
        ...
        return Transformer
    elif config.MODEL == "simple_MLP_wRes":
        return SimpleMLP
    elif config.MODEL == "simple_MLP_woRes":
        return SimpleSequentialMLP
    elif config.MODEL == "emb_simple_MLP_woRes":
        return EmbedSimpleMLP
    elif config.MODEL == "tfno":
        return TFNOModel
    else:
        raise ValueError(
...
class TFNOModel(Base):
    def __init__(
        ...
        **kwargs
        ...
        self.operator = TFNO(
            ...
            in_channels=self.input_dim,  # based on embedded inputs
        ...
        # Add dummy readout so mup can work
        self.readout = nn.Linear(1, 1)  # shape is overwritten by mup anyway
    ...
    def forward_with_embeddings(self, x, embs, return_shape='task', apply_transform=True):
        ...
        # Expand to match TFNO expected dimensions: (B, C, *spatial)
        x = x.unsqueeze(-1)  # Shape: [B, C] -> [B, C, 1]

        x = self.operator(x)  # Output shape: [B, 1, 1]
        x = x.squeeze(-1)     # Remove last dim → [B, 1]
